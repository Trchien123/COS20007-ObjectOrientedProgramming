...
namespace LookCommandUnitTests
{
    public class Tests
    {
        ...
        {
            Assert.That((look.Execute(_player, new string[] { "look around me" })), Is.EqualTo("What do you want to look at?"));
            Assert.That((look.Execute(_player, new string[] { "hello me friend" })), Is.EqualTo("Error in look input"));
            Assert.That((look.Execute(_player, new string[] { "look at gem a b" })), Is.EqualTo("What do you want to look in?"));
            Assert.That((look.Execute(_player, new string[] {"look"})), Is.EqualTo("I don't know how to look like that!"));
            Assert.Pass();
        }
    }
}